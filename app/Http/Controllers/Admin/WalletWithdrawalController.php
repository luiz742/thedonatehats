    ...
    public function withdraw(Request $request, Wallet $wallet)
    {
        ...
        if (empty($toAddress)) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Erro: Endereço de destino não configurado corretamente.',
            ], 500);
        }
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Erro ao descriptografar a chave privada.',
            ], 500);
        }
        ...
        // Extrai apenas a última linha com JSON válido
        $lines = explode("\n", trim($output));
        $lastLine = end($lines);
        ...
        if (!$response || !isset($response['success'])) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Erro ao executar o script de saque.',
            ], 500);
        }
        ...
        if (!$response['success']) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Erro no saque: ' . $errorMessage,
            ], 500);
        }
        ...
        return response()->json([
            ...
            'message' => 'Saque realizado com sucesso. TX: ' . $response['tx_id'],
        ]);
    }
