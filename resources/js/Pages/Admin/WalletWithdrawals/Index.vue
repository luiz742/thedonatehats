...
const fundAndWithdraw = async (wallet) => {
    ...
    try {
        ...
        await router.post(route('admin.wallet.withdraw', wallet.id), { amount: wallet.balance }, {
            preserveScroll: true,
            onSuccess: (page) => {
                if (page.props.flash?.success) {
                    openMessageModal('TRX enviado e saque concluído com sucesso!')
                    wallets.value = wallets.value.filter(w => w.id !== wallet.id)
                } else {
                    openMessageModal('TRX enviado, mas houve erro ao sacar USDT.')
                }
            },
            onError: (errors) => {
                const msg = errors.message || 'Erro desconhecido no saque de USDT.'
                openMessageModal('Erro: ' + msg)
            }
        })
